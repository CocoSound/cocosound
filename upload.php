										...
										echo('<p>La taille du fichier ne doit pas dépasser 20 Mo!</p>');
											...
											if(!in_array($type_file,$ext_autorisees)){
													echo('<p>Le fichier n\'est pas valide</p>');
														...
														$artisteSansEspace=str_replace(' ','',$artiste);
														
														$query3 = $bdd->prepare("SELECT COUNT(Numero_Musique)
																				FROM musique
																				WHERE REPLACE(Titre,' ','')=? 
																				AND REPLACE(Artiste,' ','')=?"
																				);
														$query3->execute(array($titreSansEspace,$artisteSansEspace));
														...
														if($nbMusiques==0){
															...
															$query4=$bdd->prepare('INSERT INTO musique(Artiste, Titre, Genre, Chemin_Musique) 
														...
														$query5=$bdd->prepare("SELECT Numero_Musique 
																			   FROM musique 
																			   WHERE REPLACE(Titre,' ','')=? 
																			   AND REPLACE(Artiste,' ','')=?"
																			   );
														$query5->execute(array($titreSansEspace,$artisteSansEspace));
														...
														$query6=$bdd->prepare('INSERT INTO uploader(Identifiant, Numero_Musique) 
																			  VALUES(:val1, :val2)'
																			  );
